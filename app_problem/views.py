...
def problem(request):
    if request.method == 'GET':
        ...
        elif request.GET.has_key('classify'):
...
def ranking(request):
    ranking_list = UserProfile.objects.order_by('-ac_times', 'submit_times')[0:10]

    return render(request, 'ranking.html', {'rankinglist':ranking_list})
